...
class JobProcessorService {
    constructor(llmService, dbService) {
     ...
     */
    async processTranscript(jobId, transcript) {
        try {
            ...
            const { validated: sanitizedTasks, invalidDependenciesRemoved, } = validation_service_1.ValidationService.validateTasks(llmOutput);
            ...
            const cycleResult = validation_service_1.GraphService.detectCycles(sanitizedTasks);
            // Step 4: Mark cyclic tasks as error (but don't crash)
            const processedTasks = validation_service_1.GraphService.markCyclicTasksAsError(sanitizedTasks, cycleResult);
            // Step 5: Determine final status for all tasks
            const allTaskIds = new Set(processedTasks.map((t) => t.id));
            const finalTasks = processedTasks.map((task) => {
                if (task.status === "error") {
                    return task; // Keep error status from cycle detection
                }
                return {
                    ...task,
                    status: validation_service_1.GraphService.determineTaskStatus(task, allTaskIds),
                };
            });
            ...
            console.log(`[Job ${jobId}] Processing complete. Tasks: ${finalTasks.length}, Cycles: ${cycleResult.cycles.length}`);
        }
        catch (error) {
